...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  alternativeText,
  onAlternative,
  alternativeButtonClass = 'bg-slate-600 hover:bg-slate-700',
}) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center" role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-describedby="modal-message">
      <div ref={modalRef} className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <p id="modal-message" className="text-slate-600 dark:text-slate-300 mb-6 whitespace-pre-line">{message}</p>
        <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
          ...
          {onAlternative && alternativeText && (
             <button
                ...
                className={`px-4 py-2 text-white font-semibold rounded-lg ${alternativeButtonClass}`}
              >
                {alternativeText}
            </button>
          )}
          <button
            onClick={onConfirm}
            className={`px-4 py-2 text-white font-semibold rounded-lg ${confirmButtonClass}`}
          >
            {confirmText}
          </button>
        </div>
